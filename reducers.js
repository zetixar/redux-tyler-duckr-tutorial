...
/*
* here we are assuming that there are orders between
* calling these reducers e.g. when we AUTH_USER we are
* assuming that error is going to be empty. Or maybe we are assuming that handling 'error' is not the bussiness of this
action/reducer to worry about. In any case, How we are ristricting the app from calling an action when it shouldn't?
e.g. User is sending a request to be authenticated, so we dispatch FETCHING_USER
*
*
